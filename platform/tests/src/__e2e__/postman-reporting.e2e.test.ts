...
describe("Postman / reporting (1:1 generated)", () => {
  ...
  it("getOfferPerformanceReport", async () => {
    const url = sub("{{baseUrl}}/v1/reports/offer-performance?journeyId={{journeyId}}&period={{period}}");
  ...
  it("getAuditStatement", async () => {
    const url = sub("{{baseUrl}}/v1/reports/audit-statement?journeyId={{journeyId}}&period={{period}}&format={{format}}");
    const res = await fetch(url, {
      method: "GET",
      headers: vars.accessToken ? { Authorization: `Bearer ${vars.accessToken}` } : {},
    });
  ...
  it("compareJourneys", async () => {
    const url = sub("{{baseUrl}}/v1/reports/journey-compare?period={{period}}");
    const res = await fetch(url, {
      method: "GET",
      headers: vars.accessToken ? { Authorization: `Bearer ${vars.accessToken}` } : {},
    });
