 ...
 * Journeys Service
 *
 * API client for journeys domain.
...
const rawJourneysService = {
   ...
   */
  async getJourney(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/journeys` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async createJourney(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/journeys`;
   ...
   * Get a journey
   */
  async getJourney(journeyId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/journeys/${journeyId}` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async updateJourney(journeyId: string, data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/journeys/${journeyId}`;
   ...
   * Archive a journey
   */
  async getArchive(journeyId: string, data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/journeys/${journeyId}/archive`;
