...
const rawConsentsService = {
  ...
  async getConsent(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    ...
    const url = `/orgs/${orgId}//v1/consents` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async getConsent(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/consents`;
  ...
  async getConsent(consentId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    ...
    const url = `/orgs/${orgId}//v1/consents/${consentId}` + (params ? `?${new URLSearchParams(params).toString()}` : '');
  ...
  async getRevoke(consentId: string, data?: any, signal?: AbortSignal): Promise<any> {
    ...
    const url = `/orgs/${orgId}//v1/consents/${consentId}/revoke`;
