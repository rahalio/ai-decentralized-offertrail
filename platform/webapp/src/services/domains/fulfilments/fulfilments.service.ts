 ...
 * Fulfilments Service
 *
 * API client for fulfilments domain.
...
const rawFulfilmentsService = {
  /**
   * List fulfilment proofs (optional failure queue filter)
   */
  async getFulfilment(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/fulfilments` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async getFulfilment(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/fulfilments`;
   ...
   */
  async getFulfilment(fulfilmentId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/fulfilments/${fulfilmentId}` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async getRetry(fulfilmentId: string, data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/fulfilments/${fulfilmentId}/retry`;
   ...
   */
  async getFreezeCollection(offerId: string, data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/offers/${offerId}/freeze-collection`;
...
export const fulfilmentsService = makeService(rawFulfilmentsService, "fulfilments");
