...
const rawReportingService = {
   ...
   */
  async getOfferPerformance(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/reports/offer-performance` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async getAuditStatement(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/reports/audit-statement` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async getJourneyCompare(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/reports/journey-compare` + (params ? `?${new URLSearchParams(params).toString()}` : '');
